        ...
        **kwargs
        ...
        try:
            ...
            if config_obj is not None:
                ...
                # Build ImageConfig with ONLY aspect_ratio
                # Note: According to Gemini API docs and current SDK (v1.41.0), 
                # ImageConfig ONLY supports aspect_ratio parameter.
                # imageSize parameter is NOT supported in the current SDK version.
                image_config_kwargs = {}
                if aspect_ratio:
                    image_config_kwargs["aspect_ratio"] = aspect_ratio

                # Handle resolution parameter for Pro model (maps to image_size in SDK v1.57.0+)
                resolution = config.get("resolution") if config else None
                if resolution and isinstance(self.gemini_config, ProImageConfig):
                    # Map resolution values to image_size values
                    # resolution: "high", "4k", "2k", "1k" -> image_size: "4K", "2K", "1K"
                    resolution_map = {
                        "4k": "4K",
                        "2k": "2K", 
                        "1k": "1K",
                        "high": "1K",  # Default to 1K for "high"
                    }
                    image_size = resolution_map.get(resolution.lower())
                    if image_size:
                        image_config_kwargs["image_size"] = image_size
                        self.logger.info(f"Using image_size={image_size} for Pro model")
                    else:
                        self.logger.warning(
                            f"Unknown resolution value '{resolution}', using default. "
                            f"Supported values: 4k, 2k, 1k, high"
                        )
                elif resolution and not isinstance(self.gemini_config, ProImageConfig):
                    self.logger.warning(
                        f"Resolution parameter '{resolution}' is only supported by Pro model, ignoring for Flash model."
                    )
